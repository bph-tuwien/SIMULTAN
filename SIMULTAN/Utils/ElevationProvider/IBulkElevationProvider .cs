...
namespace SIMULTAN.Utils.ElevationProvider
{
    ...
    public interface IBulkElevationProvider : IElevationProvider
    {
        ...
        IList<double> GetElevationAtPoints(IList<ValueTuple<double, double>> coordinates, int zoomLevel = 0);
        ...
        (double[] data, int width, int height) GetTileData(ValueTuple<double, double> pointInTile, int zoomLevel = 0);
    }
}
